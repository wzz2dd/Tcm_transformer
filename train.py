...
def main():
    ...
    for epoch in range(CONFIG['epochs']):
        ...
        for disease_batch in pbar:
            ...
            loss_treatment = torch.sum((residual * sick_mask) ** 2) / (sick_mask.sum() + 1e-6) * 50.0 #100
            loss_side_effect = torch.sum((residual * healthy_mask) ** 2) / (healthy_mask.sum() + 1e-6) * 0.1 #0.5
            loss_reg = torch.mean(generated_effect ** 2) * 1e-4 #0.001
    ...
    save_path = "checkpoints/transformer_herb_generator_nozero.pth"
